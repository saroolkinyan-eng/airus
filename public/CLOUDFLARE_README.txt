...
=== ВАЖНО: v3 ===
Если /api/health показывает "session":"missing", запустите FIX_SESSION_SECRET_WINDOWS.cmd.
Скрипт создаёт новый случайный SESSION_SECRET и передаёт его в тот же deployment через Wrangler --secrets-file.
После выполнения обновите /api/health: должно быть "session":"configured".
